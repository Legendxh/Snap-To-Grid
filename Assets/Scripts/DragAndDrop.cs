...
public class DragAndDrop : MonoBehaviour {
		...
		{ 
				...
				if (FindObjectOfType<Grid> ().occupiedPositions.Contains (lastPos)) {
						FindObjectOfType<Grid> ().occupiedPositions.Remove (lastPos);
				}
		}
		...
		void OnMouseUp()
		{
				...
				if (considerOtherObjects) {
						...
						// If it is not busy, the last saved position is removed
						// Si no esta ocupado, se remueve la ultima posicion guardada
						if (!IsOccupied ()) {
								if (FindObjectOfType<Grid> ().occupiedPositions.Contains (lastPos)) {
										FindObjectOfType<Grid> ().occupiedPositions.Remove (lastPos);
								}
								...
								if (!FindObjectOfType<Grid> ().occupiedPositions.Contains (lastPos)) {
										FindObjectOfType<Grid> ().occupiedPositions.Add (lastPos);
								}
						}
				}
		}
		...
		void UpdatePosition () {
				if (FindObjectOfType<Grid> ().occupiedPositions.Contains (currentPosition)) {
					FindObjectOfType<Grid> ().occupiedPositions.Remove (currentPosition);
				}
				...
				FindObjectOfType<Grid> ().occupiedPositions.Add (currentPosition);
		}
